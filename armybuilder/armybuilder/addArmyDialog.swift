      ...
      ScrollView(.vertical) {
        VStack(alignment: .center) {
          VStack(alignment: .leading) {
            HStack {
              Text("Select your faction ").fontWeight(.regular).padding([
            ...
            Spacer()
          }.navigationTitle("Add a new army!").padding(.top).frame(
            width: 400, height: 50, alignment: .center)
          VStack(alignment: .center) {
            NavigationLink(
              destination: selectTroops(
                factionfile: factionfile,
                collectionShowcase: collectionDatas.emptyChecker(factionID: factionfile)
              ).environmentObject(pointTargetObj).environmentObject(collectionDatas)
                .environmentObject(armyControl).environmentObject(viewControl)
                .navigationBarBackButtonHidden(true), tag: true, selection: $showNextStep
            ) {
              EmptyView()
            ...
            ForEach(factions) { faction in
              ZStack {
                Rectangle().fill(Color(UIColor.systemGray4)).frame(width: 370.0, height: 55.0)
