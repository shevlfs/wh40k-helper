  ...
  var body: some View {
    HStack {
      NavigationLink(
        ...
        HStack {
          HStack(alignment: .center, spacing: nil) {
            Group {
              Text("\(globalstats[factionID].units[id].m)".suffix(2)).font(.title2).fontWeight(
                .bold
              )
              .foregroundColor(.white).frame(maxWidth: 50, alignment: .leading).minimumScaleFactor(
                0.01
              ).lineLimit(1).frame(maxWidth: 50, alignment: .leading)
      ...
      Text("\(globalstats[factionID].units[id].name)").fontWeight(.bold).multilineTextAlignment(
        .trailing
      ).foregroundColor(.black).font(.title2).frame(maxWidth: 80, alignment: .trailing).lineLimit(3)
